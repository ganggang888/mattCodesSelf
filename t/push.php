...
if ($_GET['key'] != md5('gangganghaoshuai')){
    exit;
}
define('SEND_MESSAGE',$_GET['message']);
define('APPKEY','CCV5pYCv4b9oU3m4Ioy5a9');
define('APPID','wuCPfFxlHt74Yzwqp5m1I6');
define('MASTERSECRET','QPR5GlbWww9D7erxVedaK1');
define('CID',$_GET['cid']);
//define('DEVICETOKEN','');
//define('Alias','');
...
function IGtNotificationTemplateDemo(){
    ...
    $template->set_transmissionContent(SEND_MESSAGE);//透传内容
    $template->set_title(SEND_MESSAGE);//通知栏标题
    $template->set_text(SEND_MESSAGE);//通知栏内容
...
function IGtTransmissionTemplateDemo(){
    ...
    $template->set_appkey(APPKEY);//应用appkey
    $template->set_transmissionType(1);//透传消息类型
    $template->set_transmissionContent("你好吗，我的朋友们，在这里，我很高兴见到你哦");//透传内容
    //$template->set_duration(BEGINTIME,ENDTIME); //设置ANDROID客户端在此时间区间内展示消息
    //APN简单推送
//        $template = new IGtAPNTemplate();
//        $apn = new IGtAPNPayload();
