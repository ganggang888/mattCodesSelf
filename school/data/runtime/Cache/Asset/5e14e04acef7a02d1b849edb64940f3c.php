...
.eidt_uphoto .no {
    background: url("/school/tpl_admin/simpleboot/assets/images/upload_pic.jpg") no-repeat scroll center center #FBFBFB;
    border: 1px solid #CCCCCC;
    height: 98px;
    overflow: hidden;
    text-indent: -2000em;
...
.eidt_uphoto .del{
	...
	width:15px;
	height:15px;
	background:url("/school/tpl_admin/simpleboot/assets/images/upload_del.png") no-repeat;
	right:1px;
	top:1px;
	overflow:hidden;
	text-indent:-2000em;
...
/*上传选择按钮*/
#btupload,.addnew{
	background: url("/school/statics/js/swfupload/images/swfBnt.png") no-repeat; float:left; margin-right:10px;width:75px; height:28px; line-height:28px;font-weight:700; color:#fff;
}
#btupload{ 
...
<script>
    //更改上传限制  15-09-22 BY LIU
var _file_upload_limit="<?php echo $_GET['max_count']; ?>";
//默认为1个,为0则表示不限制
...
function album_cancel(obj,id,source){
	...
	if($("#aid-"+id).hasClass('selected')){
        ...
        var file_upload_limit = _file_upload_limit;//自定义上传的数量(15-9-22 BY LIU) _file_upload_limit=0为不限制数量
		if(file_upload_limit>0&&num > file_upload_limit) {alert('不能选择超过'+file_upload_limit+'个附件'); return false;}
...
var swfu = '';
	$(document).ready(function(){
		Wind.use("swfupload",GV.DIMAUB+"statics/js/swfupload/handlers.js",function(){
		      swfu = new SWFUpload({
			flash_url:"/school/statics/js/swfupload/swfupload.swf",
			...
			file_upload_limit:_file_upload_limit,//自定义上传的数量 15-09-22    BY LIU
    ...
    <div class="pop_cont">
      <div class="">
        <div class="edit_uping"> 
            ...
            <span class="num">最多上传<em id = "max_count"></em> 个附件,单文件最大 <em>20240 KB</em>，<em style="cursor: help;" title="可上传格式：jpg,jpeg,gif,png,bmp">支持格式？</em></span>
        </div>
...
//设置上传的最大数量，为0表示不限制数量 15-09-22 BY LIU
