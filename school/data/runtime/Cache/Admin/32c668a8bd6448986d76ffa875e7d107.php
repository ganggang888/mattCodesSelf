...
<div class="wrap J_check_wrap">
    <ul class="nav nav-tabs">
        ...
        <li><a href="<?php echo U('Sdtmain/add');?>" target="_self">添加学员</a></li>
    </ul>
    <form class="well form-search" method="post" action="<?php echo U('Sdtmain/index');?>">
    ...
    <form class="J_ajaxForm" action="" method="post">
        <div class="table-actions">
            ...
            <button class="btn btn-primary btn-small J_ajax_submit_btn" type="submit" data-action="<?php echo U('Sdtmain/delete');?>" data-subcheck="true" data-msg="你确定删除吗？">删除</button>
        </div>
        <table class="table table-hover table-bordered table-list">
            ...
            <?php if(is_array($students)): foreach($students as $key=>$vo): ?><tr>
                    ...
                    <td><?php echo ($gender = ($vo['gender'] == 0)?'女':'男'); ?></td>
                    <td><?php echo ($vo["id_card"]); ?></td>
                    <td><a href="<?php echo U('Sdtmain/result_index',array('id'=>$vo['id']));?>">查看</a></td>
                    <?php if(!empty($vo['photo'])){ ?>
                    <td><a href="<?php echo ($vo["photo"]); ?>" target='_blank'>点击查看</a></td>
                    <?php }else{ ?>
                    <td>未上传</td>
                    <?php } ?>
                    <?php $cer_array = json_decode($vo['certificate'],true); ?>
                    <td>
                        <?php if(!empty($vo['work'])): ?><a href="<?php echo U('Sdtmain/work_index',array('id'=>$vo['id']));?>">查看</a>
                            <a href="<?php echo U('Sdtmain/work_add',array('id'=>$vo['id']));?>">添加</a>

                        <?php else: ?>
                            无记录
                            <a href="<?php echo U('Sdtmain/work_add',array('id'=>$vo['id']));?>">添加</a><?php endif; ?>
                    </td>
                    <td>
                        <?php if(is_array($cer_array)): foreach($cer_array as $key=>$cer): ?><a><?php echo ($cer); ?></a><br/><?php endforeach; endif; ?>
                    </td>
                    <td>
                        <a href="<?php echo U('Sdtmain/edit',array('id'=>$vo['id']));?>">修改</a>
                        <a href="<?php echo U('Sdtmain/delete',array('id'=>$vo['id']));?>" class="J_ajax_del">删除</a>
                    </td>
