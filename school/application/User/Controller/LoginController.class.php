...
class LoginController extends HomeBaseController {
    ...
    public function findPassword()
    {
        if (IS_POST) {
            ...
            $row = $users->where(array('user_login'=>$phone))->save(array('user_pass'=>sp_password($password1)));
            if ($row) {
                $this->success('修改成功');
            } else {
                $this->error('修改失败');
...
hello;
    ...
    function dologin(){
    			...
    			switch ($uc_uid){
    				case "-1"://用户不存在，或者被删除
    					if($result){//本应用已经有这个用户
    						if($result['user_pass'] == sp_password($password) || $result['user_pass'] == md5($password)){//本应用已经有这个用户,且密码正确，同步用户
    	...
    	if($result != null)
    	{
    		if($result['user_pass'] == sp_password($password)|| $ucenter_login_ok || $result['user_pass'] == md5($password))
    		{
