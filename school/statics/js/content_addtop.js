...
//多图上传，SWF回调函数,适用于微信自定义回复
function change_imagess(uploadid, returnid) {
    var d = uploadid.iframe.contentWindow;
    var in_content = d.$("#att-status").html().substring(1);
    var in_filename = d.$("#att-name").html().substring(1);
    var str = $('#' + returnid).html();
    ...
    $.each(contents, function(i, n) {
        var ids = parseInt(Math.random() * 10000 + 10 * i);
        var filename = filenames[i].substr(0, filenames[i].indexOf('.'));
        str += "<li id='image" + ids + "'><input title='双击查看' type='text' name='" + returnid + "_url[]' value='" + n + "' style='width:310px;' ondblclick='image_priview(this.value);' class='input'> URL:<input type='text' name='" + returnid + "_alt[]' value='" + filename + "' style='width:160px;' class='input' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\"> 标题: <input type=\"text\" name=\"title[]\"> 描述: <textarea name=\"description[]\" id=\"description\" required=\"\" style=\"width:230px;height:50px;\" placeholder=\"请填写文本消息\"></textarea><a href=\"javascript:remove_div('image" + ids + "')\">移除</a> </li>";
    });
