        ...
        <div> <a href="Mymattsix.html"><img src="/new/tpl/simplebootx/Public/images/index6.png" width="100%"></a> <span class="conss">
          <h4>妈妈的分享</h4>
			...
			<div class="contatop textkey line   ">
                ...
                <?php if(is_array($lists['posts'])): $i = 0; $__LIST__ = $lists['posts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($key != 0): ?><br/><br/><br/><?php endif; ?>
                <?php $smeta=json_decode($vo['smeta'], true); ?>
                <a href="<?php echo leuu('portal/article/index',array('id'=>$vo['tid']));?>"><h1 <?php if($key == 0): ?>class="text-main"<?php endif; ?>><?php echo ($vo["post_title"]); ?></h1></a>
                <span class="text-big text-gray "><?php echo ($vo["post_keywords"]); ?></span>
					<br/>
					<br/>
                    <?php if(!empty($smeta['thumb'])): ?><span class="xm0"><img src="<?php echo sp_get_asset_upload_path($smeta['thumb']);?>" width="100%"></span>
				<br/>
				<br/><?php endif; ?>
                    <?php $content = strip_tags($vo['post_content']); ?>
                <?php if(strlen($content) > 200): ?><p><?php echo mb_substr($content,0,200,'utf-8');?>...</p>
                <?php else: ?>
                <?php echo ($content); endif; endforeach; endif; else: echo "" ;endif; ?>
                <?php echo ($lists['page']); ?>
				</div>
				
			
				<?php $resultInfo = termInfo(6); ?>
			<div class="table-responsive xm3  float-right">
                   <table class="table1">
                   <?php if(is_array($resultInfo)): foreach($resultInfo as $key=>$vo): ?><tr><td><a href="<?php echo leuu('portal/list/index',array('id'=>$vo['term_id']));?>" <?php if($term_id == $vo['term_id']): ?>class="text-main"<?php endif; ?>><h3><?php echo ($vo["name"]); ?></h3></a></td></tr><?php endforeach; endif; ?>
                   </table>
           </div>
