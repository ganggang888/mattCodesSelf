...
<body class="J_scroll_fixed">
	<div class="wrap J_check_wrap">
		<ul class="nav nav-tabs">
			...
			<li><a href="<?php echo U('Vcr/add');?>" target="_self">添加vcr</a></li>
		</ul>
		...
		<form class="J_ajaxForm" action="" method="post">
			...
			<table class="table table-hover table-bordered table-list">
				...
				<?php if(is_array($rows)): foreach($rows as $key=>$vo): ?><tr>
					...
					<td>
                    <?php if($vo['type'] == '1'): ?>教师vcr
                    <?php else: ?>
                    学生vcr<?php endif; ?>
                    <?php echo ($vo["background"]); ?></td>
					<td><?php echo strip_tags($vo[about]);?></td>
					<td><?php echo ($vo["media"]); ?></td>
					<td>
                    <?php if($vo['img'] == ''): else: ?>
                    <img src="<?php echo ($vo["img"]); ?>" width="100"><?php endif; ?></td>
					<td><?php echo ($vo["addtime"]); ?></td>
                    <td>
						<a href="<?php echo U('Vcr/edit',array('id'=>$vo['vid']));?>">修改</a> | 
						<a href="<?php echo U('Vcr/delete',array('id'=>$vo['vid']));?>" class="J_ajax_del">删除</a></td>
