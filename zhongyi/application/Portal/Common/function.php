...
function sp_sql_posts_paged($times,$tag,$pagesize=20,$pagetpl='{first}{prev}{liststart}{list}{listend}{next}{last}'){
	$where=array();
	$tag=sp_param_lable($tag);
	$field = !empty($tag['field']) ? $tag['field'] : '*';
	$limit = !empty($tag['limit']) ? $tag['limit'] : '';
	...
	if (isset($times)) {
		...
		$end = $times." 23:59:59";
		$where['post_modified'] = array('between',array($begin,$end));
		$where['term_id'] = array('in',$tag['cid']);
	}
