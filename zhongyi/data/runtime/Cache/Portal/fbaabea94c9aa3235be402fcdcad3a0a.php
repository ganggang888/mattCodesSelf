...
<?php $now_term = sp_get_term($term_id); $pid = $now_term['parent']; if($pid != 0){ $p_term = sp_get_term($pid); $grand_id = $p_term['parent']; if($grand_id != 0){ $grand = sp_get_term($grand_id); } } ?>
	...
	<div class="main main-jf clearfix">
		<div class="left">
			<?php $lists = sp_sql_posts_paged($_GET['time'],"cid:$cat_id;order:post_date DESC;",10); ?>
                ...
                <?php if(!empty($smeta['thumb'])): ?><dt>
						<img src="<?php echo sp_get_asset_upload_path($smeta['thumb']);?>" width="140" height="140" alt="">
					</dt><?php endif; ?>
				<dd>
                <?php if($term_id == 7): ?><h5><?php echo ($vo["post_title"]); ?><span class="fr">门诊时间：<?php echo ($vo["start"]); ?>-<?php echo ($vo["close"]); ?></span></h5>
                <?php else: ?>
                <h5><?php echo ($vo["post_title"]); ?><span class="fr">发布时间：<?php echo ($vo["post_modified"]); ?></span></h5><?php endif; ?>
					<p>
