		...
		<form class="form-horizontal J_ajaxForm" name="myform" id="myform" action="<?php echo u('AdminTerm/edit_post');?>" method="post">
		 <input type="hidden" name="term_id" value="<?php echo ($data["term_id"]); ?>" />
		          ...
		          <div class="tab-pane active" id="A">
		          	<table cellpadding="2" cellspacing="2" width="100%">
							<tbody>
                                ...
                                <tr>
									<td>缩略图:</td>
									<td><div  style="text-align: left;"><input type='hidden' name='img' id='thumb' value="<?php echo ((isset($data["img"]) && ($data["img"] !== ""))?($data["img"]):''); ?>">
			<a href='javascript:void(0);' onClick="flashupload('thumb_images', '附件上传','thumb',thumb_images,'1,jpg|jpeg|gif|png|bmp,1,,,1','','','');return false;">
			
			<?php if(empty($data['img'])): ?><img src="/zhongyi/statics/images/icon/upload-pic.png" id='thumb_preview' width='135' height='113' style='cursor:hand' />
			<?php else: ?>
				<img src="<?php echo sp_get_asset_upload_path($data['img']);?>" id='thumb_preview' width='135' height='113' style='cursor:hand' /><?php endif; ?>
            ...
            <input type="button"  class="btn" onClick="$('#thumb_preview').attr('src','/zhongyi/statics/images/icon/upload-pic.png');$('#thumb').val('');return false;" value="取消图片">
              ...
              <?php if(is_array($tag)): foreach($tag as $key=>$vo): ?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo['id'] == $data['tag']): ?>selected<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; ?>
              </select></td>
								</tr>
								<tr>
									<td>类型:</td>
