...
<?php $now_term = sp_get_term($term_id); $pid = $now_term['parent']; if($pid != 0){ $p_term = sp_get_term($pid); $grand_id = $p_term['parent']; if($grand_id != 0){ $grand = sp_get_term($grand_id); } } ?>
	...
	<div class="bread">
		...
		<div class="fr">
			<a href="<?php echo ($Tink["const"]["WEB_PATH"]); ?>">首页</a>>
            <?php if(!empty($grand)): echo ($grand["name"]); ?> > <?php echo ($p_term["name"]); ?> > <a href="index.php?g=portal&m=list&a=index&id=<?php echo ($now_term["term_id"]); ?>"><?php echo ($now_term["name"]); ?></a>
